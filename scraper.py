...
with open('player_data.csv', 'w', newline='') as file:
    ...
    writer.writerow(["Player ID", "Name", "Total minutes played", "Birthday", "Team", "Nationality", "Total games"])
    ...
    for player_id in range(int(start_id), int(start_id) + 500):
        ...
        try:
            ...
            writer.writerow([player_id, name, total_minutes_played, birthday, team, nationality, total_games])
            print(f"Data successfully written for player with ID: {player_id}")
